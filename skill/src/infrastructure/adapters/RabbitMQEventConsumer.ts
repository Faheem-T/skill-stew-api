import type { AnyAppEvent, AppEvent } from "@skillstew/common";
import {
  EventName,
  EventSchemas,
...
export class RabbitMQEventConsumer implements IEventConsumer {
  ...
  private handleEvent = async (msg: ConsumeMessage | null) => {
    ...
    if (!schema) {
      this._logger.error(`Unknown event: ${eventName}`, { event: msg });
    ...
    if (!parseResult.success) {
      this._logger.error(`Invalid Event payload: ${eventName}`, { event: msg });
      this._channel.nack(msg, false, false);
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        this._channel.nack(msg, false, result.retryable);
      }
    } catch (err) {
