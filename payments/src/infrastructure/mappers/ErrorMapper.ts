 ...
 */
export function mapDrizzleError(err: unknown) {
  ...
  if (!pgError) {
    ...
    return new DbQueryError("Unknown database error", err as Error);
  }
  ...
  if (code === "23503") {
    return new DbQueryError("Foreign key constraint violation", pgError);
  }
